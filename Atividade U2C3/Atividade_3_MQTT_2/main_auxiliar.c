...
int64_t turn_on_green(alarm_id_t id, void *data) {
    set_rgb_pwm(0, 65535, 0); // Liga LED verde
    return 0;
}
...
void tratar_mensagem(MensagemWiFi msg) {
    ...
    if (msg.tentativa == 0x9999) {
        if (msg.status == 0) {
            ...
            for (int i = 0; i < 3; i++)
            {
                color[i] = numero_aleatorio(0, 65535); // Gera valores aleatórios para RGB
            }
            
            set_rgb_pwm(color[0], color[1], color[2]); 
            add_alarm_in_ms(1000, turn_on_green, NULL, true); // Reseta LED após 1 segundo
        } else {
