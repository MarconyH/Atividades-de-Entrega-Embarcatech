...
#define LED_TEST_BODY "<html><body><h1>Hello from Pico.</h1><p>Actual Temperature is %.2f<p>Led is %s</p><p><a href=\"?led=%d\">Turn led %s</a></body></html>"
    ...
    if (strncmp(request, LED_TEST, sizeof(LED_TEST) - 1) == 0) {
        ...
        if (params) {
            ...
            if (led_param == 1) {
                if (led_state) {
                    // Turn led on
                    turn_on_led(LED_GPIO);
                } else {
                    // Turn led off
                    turn_off_led(LED_GPIO);
                }
            }
        }
        // Generate result
        float temperature = get_temperature();
        DEBUG_printf("Temperature: %0.2f\n", temperature);
        if (led_state) {
            len = snprintf(result, max_result_len, LED_TEST_BODY, temperature, "ON", 0, "OFF");
        } else {
            len = snprintf(result, max_result_len, LED_TEST_BODY, temperature, "OFF", 1, "ON");
        }
    }
    return len;
}
...
int main() {
    ...
    initialize_adc();
    initialize_leds((uint[]){LED_GPIO}, 1);
