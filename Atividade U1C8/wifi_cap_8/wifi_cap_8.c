...
#define LED_TEST_BODY "<html><body><h1>Hello from Pico.</h1><p>Alarm is %s</p><p><a href=\"?led=%d\">%s</a></body></html>"
...
repeating_timer_t emergency_timer;

bool emergency_timer_callback(repeating_timer_t *rt)
{
    if (emergency)
    {
...
static int test_server_content(const char *request, const char *params, char *result, size_t max_result_len)
{
    ...
    if (strncmp(request, LED_TEST, sizeof(LED_TEST) - 1) == 0)
    {
        ...
        if (params)
        {
            ...
            if (led_param == 1)
            {
                if (led_state)
                {
                    emergency = true;
                    print_text("Evacuar");
                    add_repeating_timer_ms(200, emergency_timer_callback, NULL, &emergency_timer);
                }
                else
                {
                    emergency = false;
                    cancel_repeating_timer(&emergency_timer);
                    turn_off_led(13);
                    stop_buzzer(10);
        ...
        if (led_state)
        {
            len = snprintf(result, max_result_len, LED_TEST_BODY, "ON", 0, "Desligar");
        }
        else
        {
            len = snprintf(result, max_result_len, LED_TEST_BODY, "OFF", 1, "Ligar");
        }
    }
    return len;
}
...
int main()
{
    ...
    init_display(14, 15);
    initialize_leds((uint[]){13}, 1);
    pwm_init_buzzer(10, 20000);
